...
public:
	...
	/// As runSpeedToPosition but it is blocking like runToPosition
	void runSpeedUntilPosition();
	...
	/// Added by Francesco Pudda
	/// \return the minimum pulse width in microseconds
...
private:
	/// Added by Francesco Pudda
	/// To avoid the delay call in step1 I've created these
	/// two variables that store if the step pin is HIGH
	/// and the last time it was set to HIGH in microseconds
	bool _stepPinUp = false;
	unsigned long _startPinUp;
